<template>
  <div class="mb-2" style="box-shadow: 0px 0px 1px grey">
    <v-list-item
      ...
      <v-sheet v-if="type == 'Ready'" :color="(EndTime)?'red lighten-2':((appointment.type == 'consult')?'teal lighten-2':'blue lighten-1')" height="100" width="50" class="mr-0 pa-3 d-flex justify-center" >
        <v-icon color="white">
          mdi-account-box-outline
        </v-icon>
      </v-sheet>

      <v-sheet v-else-if="CheckReadyTakeUp && type == 'Waiting'" color="teal lighten-2" height="100" width="50" class="pa-3 d-flex justify-center" >
...
<!--          {{ AppointmentTimeRemain(appointment.date + ' ' + appointment.start,appointment.date + ' ' + appointment.end)  }}-->
      ...
      <v-list-item-avatar  :class="{'mx-3': CheckReadyTakeUp || type == 'Ready'}" size="60">
        <img
      ...
      <v-list-item-content>
        <v-list-item-title class="font-weight-bold">{{ appointment.patient_firstname + ' ' + appointment.patient_lastname}}</v-list-item-title>
      ...
      <v-list-item-content v-if="type == `Ready`" class="font-weight-bold">
        <v-list-item-title>From {{ appointment.start.substr(0,5) }}</v-list-item-title>
        <v-list-item-title>To {{ appointment.end.substr(0,5) }}</v-list-item-title>
      </v-list-item-content>
      ...
      <v-list-item-content v-if="type == 'Ready'">
        <v-progress-circular
            ...
            :width="10"
            :value="time"
            :color="(EndTime)?'red':'teal'"
        >
          <v-icon :color="(EndTime)?'red':'teal'" large>
            {{ AppointmentTimeRemain(appointment.date + ' ' + appointment.start,appointment.date + ' ' + appointment.end) }}
          </v-icon>
        </v-progress-circular>
      </v-list-item-content>
