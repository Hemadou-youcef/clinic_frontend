    ...
    <v-container>
      <v-subheader>
        <router-link :to="`/dashboard`" style="text-decoration: none ;">
          <v-icon
            :color="`${mode ? '#017e5e' : 'primary darken-4'}`"
            class="mr-2"
          >
            mdi-home
          </v-icon>
...
<!--        </div>-->
        ...
        <v-row justify="center" align="center">
          <v-col
            ...
            lg="3"
            xl="2"
            v-for="patient_info in patientsList"
            :key="patient_info.id"
          >
            <patient-card
              @confirmDialog="showDeleteConfirmationDialog($event)"
      ...
      <v-dialog width="500" v-model="addPatientDialog">
      ...
      <v-dialog width="400" v-model="deletePatientDialog">
        <v-card style="overflow: hidden !important" width="400" height="150">
          <div
            class="text-caption text-center pt-6 red--text"
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on, attrs }">
                  <v-btn
                    ...
                    large
                    color="red"
                    ><v-icon color="red">mdi-check</v-icon></v-btn
                  >
                </template>
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on, attrs }">
                  <v-btn
                    ...
                    large
                    color="green"
                    ><v-icon color="green">mdi-close</v-icon></v-btn
                  >
                </template>
                <span>Cancel</span>
              </v-tooltip>
...
export default {
  ...
  methods: {
    ...
    searchPatient() {
      this.loadingButton = true;
      this.$store
        .dispatch("searchPatient", this.searchQuery)
        .then((res) => {
          this.handerResponse(res);
        })
        .catch((err) => {
